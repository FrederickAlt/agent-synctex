...
const CompileLatexFileParams = Type.Object(
	{
		...
		continuous: Type.Optional(Type.Boolean({
			description: "When true, immediately compile then subscribe this session to one shared host-service latexmk -pvc compiler for the normalized root file; latexmk handles multi-file dependency tracking with recorder/SyncTeX-friendly flags and no latexmk-owned viewer. When false, immediately compile then unsubscribe this session, stopping the compiler only when no other sessions remain. Omit continuous to leave continuous compilation unchanged.",
		})),
	},
