...
test("Viewer Host-served Viewer Client toggles reverse SyncTeX hover overlay without storing hover events", async () => {
	...
	try {
		...
		assert.match(hoverLabel, /nearest candidate: line 99 score 1000/);
		assert.match(await page.locator("[data-reverse-synctex-hover='label']").textContent() ?? "", /repair: line 4 score 4 .*\[verified\]/);
