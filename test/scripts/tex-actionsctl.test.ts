...
test("tex-actionsctl setup renders absolute paths into managed systemd unit", async () => {
	...
	assert.equal(unit.includes(`${repoRoot}/scripts/tex-actionsctl.ts daemon --socket /tmp/tex-actionsctl-setup.sock`), true);
	...
	assert.equal(outcome.codexConfigPath, resolve(base, ".codex", "config.toml"));
	const codexConfig = readFileSync(outcome.codexConfigPath, "utf8");
	assert.match(codexConfig, /\[mcp_servers\.pdf-preview\]/);
	assert.match(codexConfig, new RegExp(`command = "${context.nodePath.replaceAll("/", "\\/")}"`));
	assert.equal(codexConfig.includes(`args = ["${repoRoot}/scripts/tex-actions-mcp.ts"]`), true);
	assert.match(codexConfig, /\[mcp_servers\.pdf-preview\.env\]/);
	assert.equal(codexConfig.includes('TEX_ACTIONS_HOST_SERVICE_SOCKET_PATH = "/tmp/tex-actionsctl-setup.sock"'), true);
